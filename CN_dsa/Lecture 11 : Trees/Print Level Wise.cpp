...
void printLevelWise(TreeNode<int>* root) {
    ...
    que.push(temp);
    while(que.size()!=0){
        TreeNode<int>*bro=que.front();
        que.pop();
        cout<<bro->data<<':';
        for(int i=0;i<bro->children.size();i++){
            if(i==(bro->children.size()-1)){
                cout<<bro->children[i]->data;
            }else{
            cout<<bro->children[i]->data<<',';
            }
        }
        cout<<endl;
        // cout<<bro->children.back()->data<<endl;
        
        for(int i=0;i<bro->children.size();i++){
            que.push(bro->children[i]);
        }
