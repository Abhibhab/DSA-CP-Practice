...
void solve() {
    ...
    int chota = num;
    int bara = 1000 * num;
    if ((right < num || right > bara) || (left < num || left > bara)) {
        cout << "Impossible" << nl;
        return;
    } else {
        int lowrow = min(right, left) - ((N - 1));
        int rem = 0;
        if (lowrow > 1000 * M) {
        ...
        // debug(sara);
        int leftright = max(0, right - sara);
        int leftleft = max(0, left - sara);
        int bro = leftright / (N - 1);
        int brorem = leftright % (N - 1);
        int sis = leftleft / (N - 1);
        int sisrem = leftleft % (N - 1);
        fr (i, 0, N - 1) {
            arr[i][0] = bro;
