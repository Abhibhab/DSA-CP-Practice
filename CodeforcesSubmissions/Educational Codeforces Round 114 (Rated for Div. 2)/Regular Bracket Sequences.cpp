...
void generateboroParenthesis(int n, int open, int close, string s, vector<string> &ans) {
    ...
    if (open < n) {
        generateboroParenthesis(n, open + 1, close, s + "(", ans);
    }

    if (close < open) {
        generateboroParenthesis(n, open, close + 1, s + ")", ans);
    }
...
void solve() {
    ...
    generateboroParenthesis(x, 0, 0, "", ans);
    for (int i = 0; i < x; i++) {
        cout << ans[i] << nl;
    }
